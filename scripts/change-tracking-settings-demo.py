...
class StrandCamProxy:
    def __init__(self, strand_cam_url):
        ...
        self.shared_state = {}
        thread = threading.Thread(target=maintain_state_copy,args=(event_iterator,self.shared_state))
...
""".format(mode=mode)
        ...
        if r.status_code != requests.codes.ok:
            print('error making request, status code {}'.format(r.status_code), file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    parser.add_argument("--strand-cam-url", type=str, default='http://127.0.0.1:3440/',
